...
public class RedisCache implements InitializingBean {
    ...
    public static class CacheKey {
        ...
        public <T> CacheList<T> list() {
            return new CacheList<>(key, ttl, instance.redissonClient.getList(key));
        }

        public <T> CacheSet<T> set() {
            return new CacheSet<>(key, ttl, instance.redissonClient.getSet(key));
        }

        public <T> CacheQueue<T> queue() {
            return new CacheQueue<>(key, ttl, instance.redissonClient.getQueue(key));
        }

        public <T> CacheBlockingQueue<T> blockingQueue() {
            return new CacheBlockingQueue<>(key, ttl, instance.redissonClient.getBlockingQueue(key));
        }

        public <T> CacheScoredSortedSet<T> scoredSortedSet() {
            return new CacheScoredSortedSet<>(key, ttl, instance.redissonClient.getScoredSortedSet(key));
        }

        public CacheMap map() {
            return new CacheMap(key, ttl, instance.redissonClient.getMap(key));
        }

        public CacheAtomicLong atomicLong() {
            return new CacheAtomicLong(key, ttl, instance.redissonClient.getAtomicLong(key));
        }
