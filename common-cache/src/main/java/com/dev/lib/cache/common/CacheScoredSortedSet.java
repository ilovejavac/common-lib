...
public class CacheScoredSortedSet<T> {
    ...
    public boolean add(double score, T object) {
        boolean result = rScoredSortedSet.add(score, object);
        if (result) {
            expireIfNeeded();
        }
        return result;
    ...
    public Double addScore(T object, Number value) {
        Double result = rScoredSortedSet.addScore(object, value);
        expireIfNeeded();
        return result;
    }
    ...
    public int removeRangeByRank(int startIndex, int endIndex) {
        int result = rScoredSortedSet.removeRangeByRank(startIndex, endIndex);
        if (result > 0) {
            expireIfNeeded();
        }
        return result;
