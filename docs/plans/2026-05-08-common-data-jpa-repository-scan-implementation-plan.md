...
- Local install: `JAVA_HOME=$(/usr/libexec/java_home -v 25) mvn -pl common-data-jpa,common-aksk -am install -DskipTests` passed and installed updated `1.5.1-RC0` jars into `~/.m2`.
- Installed jar check: `common-aksk-1.5.1-RC0.jar` no longer contains `AkskAutoConfiguration` or `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`; installed `common-data-jpa` contains the merged package scanner.
