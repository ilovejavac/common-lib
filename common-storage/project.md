...
writeFile(ctx, "/data.json",inputStream);
...
copy(ctx, "/src-folder","/dest-folder",true);

// 移动/重命名
vfs.

move(ctx, "/old.txt","/new.txt");
vfs.

move(ctx, "/old-folder","/new-folder");

// 删除文件
vfs.

delete(ctx, "/file.txt",false);

// 递归删除目录
vfs.

delete(ctx, "/folder",true);
```
