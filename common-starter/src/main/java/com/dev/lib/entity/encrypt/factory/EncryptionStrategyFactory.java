...
public class EncryptionStrategyFactory {
    ...
    public EncryptService getStrategy(EncryptVersion version) {
        return getStrategy(version.getMsg());
    }

    public EncryptService getStrategy(String version) {
        EncryptService service = strategies.get(version);
        if (service == null) {
     ...
     * 获取所有可用策略
     */
    public Map<String, EncryptService> getAllStrategies() {
