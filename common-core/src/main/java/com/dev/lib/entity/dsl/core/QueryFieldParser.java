...
public class QueryFieldParser {
    ...
    static {
        ...
        // EXISTS 系列
        QUERY_TYPE_SUFFIX.put("NotExists", QueryType.NOT_EXISTS);
        QUERY_TYPE_SUFFIX.put("Exists", QueryType.EXISTS);

        // NULL 系列
        QUERY_TYPE_SUFFIX.put("IsNotNull", QueryType.IS_NOT_NULL);
        QUERY_TYPE_SUFFIX.put("IsNull", QueryType.IS_NULL);

        // 字符串匹配
        QUERY_TYPE_SUFFIX.put("StartWith", QueryType.START_WITH);
        QUERY_TYPE_SUFFIX.put("EndWith", QueryType.END_WITH);
        QUERY_TYPE_SUFFIX.put("Like", QueryType.LIKE);

        // 集合
        QUERY_TYPE_SUFFIX.put("NotIn", QueryType.NOT_IN);
        QUERY_TYPE_SUFFIX.put("In", QueryType.IN);

        // 比较
        QUERY_TYPE_SUFFIX.put("Between", QueryType.BETWEEN);
        QUERY_TYPE_SUFFIX.put("Ge", QueryType.GE);
        QUERY_TYPE_SUFFIX.put("Gt", QueryType.GT);
        QUERY_TYPE_SUFFIX.put("Le", QueryType.LE);
        QUERY_TYPE_SUFFIX.put("Lt", QueryType.LT);
        QUERY_TYPE_SUFFIX.put("Ne", QueryType.NE);
        QUERY_TYPE_SUFFIX.put("Eq", QueryType.EQ);
    }
    ...
    public static ParsedField parse(String fieldName) {
        ...
        for (Map.Entry<String, QueryType> entry : QUERY_TYPE_SUFFIX.entrySet()) {
            if (remaining.endsWith(entry.getKey())) {
                ...
                remaining = remaining.substring(0, remaining.length() - entry.getKey().length());
                break;
            }
        }
        ...
        return new ParsedField(targetField, queryType, operator, isSubQuery);
    }
